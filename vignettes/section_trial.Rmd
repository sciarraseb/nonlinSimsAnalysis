...
Therefore, under equal spacing, error bar lengths for all the day-unit parameters except the random-effect triquarter-halfway elevation parameter ($\upgamma_random$) are shortest when the nature-of-change value set by the fixed-effect days-to-halfway elevation parameter ($\upbeta_{fixed}$) has a value of 180. Even though the error bar length for the random-effect triquarter-halfway elevation parameter ($\upgamma_random$) with a nature-of-change value of 80 decreases after removing the maximum-value estimates, it remains longer than the error bar lengths with the other nature-of-change values due to the existence of high-value estimates. 
...
```{r second_deriv, echo=F, eval=F}

log_func <- expression(y = t + (a - t)/((1 + exp(b - time)/g)))
second_der <-D(expr = log_func, name = 'time')
...
second_der_values <- eval(second_der)

second_der_df <- data.frame('time' = time, 
                            'second_der_value' = second_der_values)

ggplot(data = second_der_df, mapping = aes(x = time, y = second_der_value)) + 
  geom_line() + 
